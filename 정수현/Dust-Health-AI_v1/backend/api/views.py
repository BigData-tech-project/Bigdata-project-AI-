...
def fetch_and_store_air_quality(request):
    """
    도시 및 지역별 미세먼지 데이터를 API에서 가져와 DB에 저장
    """
    city = request.data.get("city")  # 요청에서 도시 이름 가져오기
    if not city:
        return JsonResponse({'success': False, 'message': '도시 이름이 필요합니다.'}, status=400)
    ...
    try:
        ...
        if response.status_code != 200:
            return JsonResponse({'success': False, 'message': 'API 요청 실패'}, status=500)

        items = response.json().get("response", {}).get("body", {}).get("items", [])
        if not items:
            return JsonResponse({'success': False, 'message': '데이터가 없습니다.'}, status=404)
        ...
        return JsonResponse({'success': True, 'message': f'{city} 데이터가 저장되었습니다.'}, status=200)
