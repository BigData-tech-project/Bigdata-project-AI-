...
const Main = () => {
  ...
  const [futureForecast, setFutureForecast] = useState(null); // 미래 예보 상태 추가
  ...
  const fetchDbData = async () => {
    try {
      ...
      console.log('DB Response:', response.data); // DB 응답 확인
  
      const [mainRegion, subRegion] = response.data.region?.split(' ') || [];
      setCity(mainRegion || '');
      setRegion(subRegion || '');
  
      console.log('City:', mainRegion, 'Region:', subRegion); // 디버깅 출력
    } catch (error) {
  ...
  const fetchHourlyForecast = async () => {
    if (!region) {
      console.error('지역 정보가 없습니다. region:', region);
      return;
    }
    ...
    try {
      ...
      console.log('Hourly Forecast API Response:', response.data); // 응답 데이터 확인
      setHourlyForecast(response?.data?.response?.body?.items || []);
    } catch (error) {
      console.error('시간별 예보 데이터 가져오기 실패:', error);
    }
  };
  ...
  try {
    ...
    const forecastItems = response.data?.response?.body?.items || [];
    const matchedForecast = forecastItems.find((item) =>
      item.informGrade.includes(city)
    );

    setFutureForecast(matchedForecast);
  } catch (error) {
        ...
        <section className="hourly-forecast">
          ...
          >
            {hourlyForecast.length > 0 ? (
              hourlyForecast.slice().reverse().map((item, index) => (
                <div key={index} className="forecast-box">
                  <p className="pmText">{item.dataTime}</p>
                  <hr />
                  <div className="emoji">{getEmoji(item.pm10Value)}</div>
                  <hr />
                  <p className="pmStatus">{item.pm10Value} µg/m³</p>
                </div>
              ))
            ) : (
