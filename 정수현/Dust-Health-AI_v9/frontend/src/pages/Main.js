...
const Main = () => {
  ...
  const getEmoji = (value) => {
    if (typeof value === 'string') {
      if (value.includes('좋음')) return '😊';
      if (value.includes('보통')) return '😐';
      if (value.includes('나쁨')) return '😷';
      if (value.includes('매우 나쁨')) return '😡';
    } else if (typeof value === 'number') {
      if (value <= 30) return '😊';
      if (value > 30 && value <= 80) return '😐';
      if (value > 80 && value <= 150) return '😷';
      if (value > 150) return '😡';
    }
    return '❓'; // 예외 처리: 적합한 값을 찾지 못했을 때
  };
