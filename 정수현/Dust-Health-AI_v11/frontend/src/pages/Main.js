...
const Main = ({setIsAuthenticated}) => {
  ...
  const { pastdustData } = useContext(DustDataContext);
  ...
  const fetchFuturePrediction = async () => {
    ...
    try {
      ...
      console.error('미래 예측 데이터를 가져오는데 실패했습니다:', error);
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const fetchFutureForecast = async () => {
    ...
    try {
      ...
      const relevantForecast = forecastItems.find((item) =>
        item.informGrade.includes(city)
      );

      if (relevantForecast) {
        setFutureForecast(relevantForecast);
      } else {
        console.warn('해당 지역에 대한 미래 예보가 없습니다.');
        setFutureForecast(null);
      }
    } catch (error) {
      console.error('미래 예보 데이터를 가져오는데 실패했습니다:', error);
      setFutureForecast(null);
    }
  };
  ...
  const renderFuturePrediction = () => {
    if (filteredPrediction && filteredPrediction.future_dates?.length > 0) {
    ...
    if (futureForecast) {
      const forecastDate = futureForecast.dataTime?.split(' ')[0] || '날짜 없음';
      const grade = futureForecast.informGrade
        ?.split(',')
        .find((g) => g.includes(city))
        ?.split(':')[1]
        ?.trim();
