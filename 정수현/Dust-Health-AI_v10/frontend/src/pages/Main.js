...
const Main = () => {
  ...
  const { pastdustData, todaydustData, futureForecast,  filteredPrediction } = useContext(DustDataContext);
  ...
  const renderFuturePrediction = () => {
    if (filteredPrediction && filteredPrediction.future_dates?.length > 0) {
      // filteredPrediction 값이 있을 경우 이를 표시
    ...
    if (futureForecast) {
      // filteredPrediction이 없거나 데이터가 비어 있을 경우 futureForecast로 대체
      const forecastDate = futureForecast.dataTime?.split(' ')[0];
      const grade = futureForecast.informGrade
        .split(',')
        .find((grade) => grade.includes(city))?.split(':')[1]?.trim();
  
      return (
        <div className="forecast-box">
          <p className="pmText">{forecastDate || "예보 날짜 없음"}</p>
          <hr />
